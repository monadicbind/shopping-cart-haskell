...
doveProduct :: Product Offer
doveProduct = createAProduct "Dove Soap" 39.99 (Nothing :: Maybe Offer)

doveSoapWithOffer :: Offer -> Product Offer
doveSoapWithOffer offer = createAProduct "Dove Soap" 39.99 (Just offer)

axeDeo :: Decimal -> Product Offer
axeDeo rate = createAProduct "Axe Deo" rate (Nothing :: Maybe Offer)

emptyCart :: Cart Offer
emptyCart = createAnEmptyCart
  ...
  describe "Shopping Cart" $ do
    ...
    describe "Adding 2 DoveSoaps and 2 Axe Deo more to an emptycart" $
      it " return a cart with 4 products" $ do
        ...
        let newCart = addProducts emptyCart doveProduct quantity
        let taxRate = 12.5
        let finalCart = addProducts newCart (axeDeo 99.99) quantity
    ...
    describe "Adding 3 DoveSoaps and 2 AxeDeo to the cart" $
      it " return a cart with 5 products" $ do
        ...
        let taxRate = 12.5
        let finalCart = addProducts dsCart (axeDeo 89.99) 2
        let testableAmounts = totalPriceWithTaxes finalCart taxRate
